...
def grasp_parcel_right(arm, claw, parcel, log, cam=None, tf_reader=None,
    ...
    hold_latched = False
    if not skip_claw:
        held, hold_latched = _await_right_hold(claw, log, "post-close")
        if not held:
    ...
    if not skip_claw:
        ...
        if hold_latched or claw.right_holding() or int(
            ...
            return _safe_abort_grasp_open(
                claw, arm, right_pre, rq, log, "empty_raise_open", False)
    else:
...
def handoff_to_left(arm, claw, log):
    ...
    log("[HANDOFF] coïncidence (angles miroir, settle=%.1fs) — puis close L", settle)
    rospy.sleep(settle)
    ...
    log("[HANDOFF] left claw L=%d — double hold avant open R", ls)

    both = float(globals().get("HANDOFF_BOTH_HOLD_BEFORE_OPEN_R", 0.5) or 0.5)
    rospy.sleep(both)
...
def run_scene1_mission(robot, arm, claw, head, log):
    ...
    while completed < MAX_PARCELS:
        ...
        if not grasped:
            ...
            max_fail = 5 if FORCE_PARCEL_NAME else 2
